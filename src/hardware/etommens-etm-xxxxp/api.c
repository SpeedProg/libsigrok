...
static const struct etommens_etm_xxxxp_model supported_models[] = {
	...
	{ 0x4B50, 3010, "eTM-3010P/RS310P/HM310P", 10, 30,  300, 3, 2, 3 },
};
...
static struct sr_dev_inst *probe_device(struct sr_modbus_dev_inst *modbus)
{
	...
	ret = etommens_etm_xxxxp_device_info_get(modbus, &modelid, &dclassid);
	if (ret != SR_OK)
		return NULL;
	for (i = 0; i < ARRAY_SIZE(supported_models); i++) {
		if (modelid == supported_models[i].modelid && dclassid == supported_models[i].classid) {
			model = &supported_models[i];
			break;
		}
		if (model == NULL) {
			sr_err("Unknown model %d and class 0x%X.", modelid, dclassid);
			return NULL;
		}
	}
		...
		const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_ENABLED:
		return etommens_etm_xxxxp_reg_set(sdi, REG_POWER_OUT, g_variant_get_boolean(data));
	case SR_CONF_OVER_VOLTAGE_PROTECTION_THRESHOLD:
		return etommens_etm_xxxxp_reg_set(sdi, REG_OVP_VALUE, g_variant_get_double(data) * devc->voltage_multiplier);
	case SR_CONF_OVER_CURRENT_PROTECTION_THRESHOLD:
		return etommens_etm_xxxxp_reg_set(sdi, REG_OCP_VALUE, g_variant_get_double(data) * devc->current_multiplier);
	default:
		return SR_ERR_NA;
	}
		...
		const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_CURRENT_LIMIT:
		*data = std_gvar_min_max_step(0.0, devc->model->max_current,
		...
		break;
	case SR_CONF_OVER_CURRENT_PROTECTION_THRESHOLD:
		*data = std_gvar_min_max_step(0.0, devc->model->max_current+0.5,
						1/devc->current_multiplier);
		break;
	case SR_CONF_OVER_VOLTAGE_PROTECTION_THRESHOLD:
		*data = std_gvar_min_max_step(0.0, devc->model->max_voltage+3,
						1/devc->voltage_multiplier);
		break;
	default:
		return SR_ERR_NA;
