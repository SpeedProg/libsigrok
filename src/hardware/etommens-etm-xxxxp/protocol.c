			   ...
			   uint16_t address, uint16_t value)
{
	...
	g_mutex_lock(&devc->rw_mutex);
	ret = sr_modbus_request_reply(modbus, request, sizeof(request), reply, sizeof(reply));
	g_mutex_unlock(&devc->rw_mutex);
	return ret;
}
 ...
 *         SR_ERR_DATA upon invalid data, or SR_ERR on failure.
 */
SR_PRIV int etommens_etm_xxxxp_device_info_get(struct sr_modbus_dev_inst *modbus, uint16_t *model, uint16_t *dclass)
{
...
SR_PRIV int etommens_etm_xxxxp_receive_data(int fd, int revents, void *cb_data)
{
	...
	ret_power = sr_modbus_read_holding_registers(modbus, REG_POWER_OUT, 1, power_register);
	ret_protect = sr_modbus_read_holding_registers(modbus, REG_PROTECTION, 1, protect_register);
	g_mutex_unlock(&devc->rw_mutex);
	if (ret == SR_OK) {
	...
	if (ret_power == SR_OK) {
		if (devc->last_output_state != RB16(power_register)) {
			...
			sr_session_send_meta(sdi, SR_CONF_ENABLED, g_variant_new_boolean(devc->last_output_state));
		}
	}
	if (ret_protect == SR_OK) {
		if (devc->last_ovp_state != (RB16(protect_register) & 0x0001)) {
			devc->last_ovp_state = (RB16(protect_register) & 0x0001);
			sr_session_send_meta(sdi, SR_CONF_OVER_VOLTAGE_PROTECTION_ACTIVE, g_variant_new_boolean(devc->last_ovp_state));
		}
		if (devc->last_ocp_state != (RB16(protect_register) & 0x0002)) {
			devc->last_ocp_state = (RB16(protect_register) & 0x0002);
			sr_session_send_meta(sdi, SR_CONF_OVER_CURRENT_PROTECTION_ACTIVE, g_variant_new_boolean(devc->last_ocp_state));
		}

		if (devc->last_otp_state != (RB16(protect_register) & 0x0008)) {
			devc->last_otp_state = (RB16(protect_register) & 0x0008);
			sr_session_send_meta(sdi, SR_CONF_OVER_TEMPERATURE_PROTECTION_ACTIVE, g_variant_new_boolean(devc->last_otp_state));
		}
